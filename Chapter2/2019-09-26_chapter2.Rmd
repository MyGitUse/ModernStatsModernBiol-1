...
likelihood = dbinom(x=rep100, prob = probs, size = length(rep100))
plot(probs, likelihood, pch = 16, xlab = "probability of success",
       ylab = "likelihood", cex=0.6)
probs[which.max(likelihood)]
```
...
```{r eval=TRUE}
staph = readBStringSet("data/staphsequence.ffn.txt", format="fasta")
...
```{r eval=TRUE}
set.seed(123456789)
sim.seq <- rbinom(1000, prob=0.0001, size=1000)
sum(sim.seq)
```

Set seed before making random variable for reproducibility.  

instead of vapply and lappy use purr package
for each instead of for loops
