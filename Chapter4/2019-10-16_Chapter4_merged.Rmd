...
&emsp; <a href="#1">[1]</a> Where might you see mixture data?  
&emsp; <a href="#2">[2]</a> What is mixture data?  
&emsp; <a href="#3">[3]</a> Chapter Goals  
&emsp; <a href="#4">[4]</a> Finite Mixtures  
&emsp; <a href="#5">[5]</a> The Expectation-Maximization (EM) Algorithm  
&emsp; <a href="#6">[6]</a> Zero inflated data  
&emsp; <a href="#7">[7]</a> More than two components  
&emsp; <a href="#8">[8]</a> Empirical distributions and the nonparametric bootstrap   
&emsp; <a href="#9">[9]</a> Why is this bootstrapping method called nonparametric?  
&emsp; <a href="#10">[10]</a> Infinite Mixtures   
&emsp;&emsp; <a href="#11">[11]</a> Infinite mixture of normals  
&emsp;&emsp; <a href="#12">[12]</a> Asymmetric Laplace  
&emsp;&emsp; <a href="#13">[13]</a> Infinite mixtures of poisson variables: the gamma-poisson distribution  
&emsp; <a href="#14">[14]</a> Choosing a data transformation based on a mixture model of your data: variance stabilizing transformations  
&emsp; <a href="#15">[15]</a> Exercises  
</br>


<p id="1"><b><font size="5">Where might you see mixture data?</b></font><a href="#0"><sup>Return</sup></a></p>
* Flow cytometry  
* RNA-seq  
* ChIP-Seq  
* Microbiome  
...
<center><button data-toggle="collapse" data-target="#demo">Spoiler</button>
<div id="demo" class="collapse">
Answer?
</div></center>

<p id="2"><b><font size="5">What is mixture data?</b></font><a href="#0"><sup>Return</sup></a></p>   
...
In some mixture data sets we are able to determine if the number of components is small and less than the number of our observations - this is called _*finite mixture data*_.  
In other mixture data sets, the number of components is equal to (or greater than) our number of observerations - this type of mixture data is called _*infinite mixture data*_.  


<p id="3"><b><font size="5">Chapter Goals</b></font><a href="#0"><sup>Return</sup></a></p>
...
<p id="4"><b><font size="5">Finite Mixtures</b></font><a href="#0"><sup>Return</sup></a></p>
...
set.seed(1234)
#coinflips flips a fair coin 10000 times - split this into logical(TRUE/FALSE) by 0.5 due to fair coin (Heads/tails)
coinflips = (runif(10000) > 0.5)
#table of true/false (heads/tail) outcome from coinflips
table(coinflips)

#one flip generates 2 normal distributions, one from a distribution with mean=1, var=0.25 (sd=0.5) [distribution1], and one from a distribution with mean=3 and var=0.25 [distribution2]
  # Think of TRUE=1 & FALSE=0
  # If the coinflip is TRUE(1) then this function will evaluate using distribution1
    # If FALSE, then distribution2
...
#Fairmix is generated by applying the oneflip function to all coinflips in the vector
...
If you thought vapply() was the best thing since for loops, you'll be excited to learn there's an even easier way to accomplish the above using rnorm(), by setting the "x=" argument to the length of the `coinflips` vector and the ifelse command.

**<span style="color:red"> *WATCH OUT*: </span>** The authors suddenly started using variance instead of standard deviation for some reason that is unknown to us. The graph outputs will vary because standard deviation gives more of a spread from the mean.

* For our example, we will show that you can generate the same graphs without using vapply(), as long as we consistently use standard deviation or varience in both methods.
...
* **NOTE** - We have to store the outputs of the simulated distributions as a dataframe, or in this case tibble() because this is what ggplot requires
...
The 2 normal distributions become more distinct with a greater number of observations, which becomes a smooth limiting curve called the density function. Therefore, it can be taken that a larger sample allows better approximation of the number of mixtures.   

But how does the distribution density of our 1000000 coin flips from the population with mean=1 (TRUE/Heads flip) look like if we converted the y-axis to a proportion of counts?

  * **NOTE** - a single NORMAL distribution in a mixture model is denoted Φ(X) where phi is the density of a normal distribution N(mu,var)
...
It looks pretty darn close to the normal distribution with y-axis of counts.  

Thus, we can write a general formula for density of any mixture model by suming the densities of individual normal distributions (proportionately).   
...
* ϕ1 is the density of normal N(μ1=1,σ<sup>2</sup>=0.25) - In this case TRUE/Heads
* ϕ2 is the density of normal N(μ1=3,σ<sup>2</sup>=0.25) - In this case FALSE/Tails

Now, what if we wanted to just visualize a theoretical density of the mixture without generating Heads/Tails and calculating proportion results from a coinflip?

```{r eval=TRUE}
# Note - seq(from -1 to 5 with 1000 variables) replaces coinflips
  # 0.5 is probability of heads/tails because of a fair coin - this gives us a distribution where probabilities sum to 1
fairtheory = tibble(
...
<mark style='background-color:red'> Why is this in here twice? </mark> 
...
Assume you didn't see the code below and only the figure. Can you guess the means of the two sample mixture below? Each component has same variance.
...
Probably not, finite mixture models be hard to distinguish if similar mixture distributions have similar means (in this case normal distribution 1 mu=1, normal distribution 2 mu=2)


Now that we know how the data was generated, we can identify the two components of the mixture.
...
Here is another method the author used to show overlap of finite mixtures
...
<p id="5"><b><font size="5">The Expectation-Maximization (EM) Algorithm</b></font><a href="#0"><sup>Return</sup></a></p>

The EM algorithm is used to infer the value of hidden components.  
...
* Pretending to know the distribution values of each component (and re-estimating the probability that each observation belongs to a given component)  
...
* Value of θ maximizing E*(expection) found in _maximization step_
  * Iterations through E & M repeated until E* increases are small -> indicates plateauing likelihood = reaching local max
    * Iteration path **depends on starting point** but always end up at the same end point (assuming 1 end point)
...
<mark style='background-color:red'>Should we present excercise4.1 here?</mark>
  
How would this work if applied to two unfair coins with p1=0.125(Coin1), p2=0.25(Coin2), probability of of picking coin 1 equal to π and probability of picking coin 2 equal to 1-π when we toss the coin twice and record the number of heads K?

* Probability of tossing coin1 in toss 1 -> π=1/8
* Probability of tossing coin2 in toss 2 -> π=1/4

```{r eval=TRUE}
#simulate 100 instances of this procedure and compute the contigency table
...
These contingecy tables look similar. Thus, it would be hard to identify the distribution parameters based on this data. This illustrates the issue  **indentifiability** -> same observed values can have several explainantions of values (p1, p2, π). This occurs when there are too many degrees of freedom in the parameters (df = n-1)

> **Why do we subtract 1 from the number of items?**  
&emsp; Another way to look at degrees of freedom is that they are the number of values that are free to vary in a data set. What does “free to vary” mean? Here’s an example using the mean (average):  
&emsp;&emsp; Q. Pick a set of numbers that have a mean (average) of 10.  
&emsp;&emsp; A. Some sets of numbers you might pick: 9, 10, 11 or 8, 10, 12 or 5, 10, 15.
...
&emsp; For example: if you wanted to find a confidence interval for a sample, degrees of freedom is n – 1. “N’ can also be the number of classes or categories. 

<mark style='background-color:red'>The follow question states means are unknown but then procceeds to provide mean of -0.5 & 1.5. Does y indicate estimated means, and if so where does mus come from?</mark>

Let's look at another example of a mixture of two normals with mean parameters unknown and standard deviation equal to 1 (μ1=?,μ2=?,σ1=σ2=1).
...
Notice in previous the `coinflips` example, the results of the vector are logical (TRUE/FALSE). Here, since the means are unknown the values are 1 or 2.  

Since we know the labels "u" (1 or 2), we can estimate the means by using seperate ML equations for each group.

* If we know the labels (u), we can estimate means using separate MLEs for each group.
  * The overall MLE obtained by maximizing the following equation. Or by its log.
...
```{r eval=TRUE}
#group the tibble `duy` at "u" column (group by 1's or 2's), then summarize the mean of the "y" column within group
...
* **NOTE** - think back to this equation
...
* Which can also be viewed as, where
  * Φ(x) = N(mu, var)
  * λ = mixture weight for an individual distribution within a mixture model

\begin{equation}
f(x)=λ\phi_1(x)+λ\phi_2(x)
\end{equation}
...
```{r eval=F}
EMtest <- Mclust(duy$y, G=2) #where G is the number of components
EMtest$n
EMtest$df
EMtest$BIC
...
mixtools <- normalmixEM(duy$y, k=2)
...
These are two different packages, but they have a very similar outcome due to convergence of log maximum likelihood.
...
The EM algorithm is very informative:
  ...
  * Example of **soft averaging** (don't need to decide if observation belongs to group 1 or 2 - observation can join multiple groups by using probability membership as weights) -> better estimates of parameters
  * Can use this method for general case of model averaging
    ...
    * Weights provided by log likelihoods of each model
    
<p id="6"><b><font size="5">Zero inflated data </b></font><a href="#0"><sup>Return</sup></a></p>
...
Where :
  ...
  * Delta = Dirac delta function -> represents probability distribution with mass at 0
...
```
<mark style='background-color:red'>Is this a significant number of zeros? The proportion of zeros in the data doesn't look significant based on the counts of other numbers (is this true - we're not sure).</mark>
 
<p id="7"><b><font size="5">More than two components</b></font><a href="#0"><sup>Return</sup></a></p>
...
What happens if we repeat this exercise with N=1000 nucleotide measurements? (decreasing the #observations)
...
* **NOTE** -  
  * μ (mean) = any real number, the location parameter
    * Location parameter tells where graph is located (Where on the horizontal axis centers the graph, relative to normal distribution model)
      * ex) normal distribution is centered at 0, so if parameter=5 -> center graph 5 units to right of 0
  * σ = the scale parameter (Only true for normal distribution)
...
<p id="8"><b><font size="5">Empirical distributions and the nonparametric bootstrap</b></font><a href="#0"><sup>Return</sup></a></p>

This type of data deals with the situation where the number of observations is equal to the number of mixture components.
...
* ECDF is a probability model for emperical data when sampling from a sample instead of the entire population
...
```{r eval=TRUE}
B = 1000
#Use Monte Carlo to draw 1000 samples of size 15 from the 15 values
...
# Bootstrap percentile/quantile method
  ...
  # In quantiles this equates to 0.005 quantile (lower) & 1-0.005 = 0.995 (upper)
  # The conclusion = IDK
...
* Bootstrap Percentile Method
  * Calcualte confidence intervals for bootstrapped samples
  1. Simple method, certain % is trimmed from lower & upper end of the sample statistic (ex) mean or sd)
...
zea_meds <- bootstrap(ZeaMays$diff, B, theta)

#plot bootstrap means
ggplot(tibble(zea_meds2=zea_meds$thetastar),aes(x = zea_meds2))+
  geom_histogram(bins = 30, fill = "purple")
```

<button data-toggle="collapse" data-target="#bootmean">Bootstrap for mean</button>
<div id="bootmean" class="collapse">
```{r eval=TRUE}
#create a theta function to bootstrap mean
set.seed(1234)
theta <- function(x){
...
<p id="9"><b><font size="5">Why is this bootstrapping method called nonparametric?</b></font><a href="#0"><sup>Return</sup></a></p>
...
* A) Errors
  * Error1: Monte Carlo simulations of subsets of data via random resampling used to approximate exhaustive bootstrap
    * By increasing the size of `nboot` argument of `boostrap()` -> this reduces Monte Carlo error
      * **Keep in mind, exhaustive boostrap is still not exact** b/c using approximate distribution function of data instead of true distribution
...
<p id="10"><b><font size="5">Infinite Mixtures</b></font><a href="#0"><sup>Return</sup></a></p>
...
<p id="11"><b><font size="4">Infinite mixture of normals</b></font><a href="#0"><sup>Return</sup></a></p>
...
* Lapace density
  ...
  * From this formula
    * Level1 (absolute value of difference) Distance has similar position in Laplace density as
    * Level2 (square of difference) in the normal density 
    
* **NOTE** - in Bayesian regression using Laplace distribution as a prior on the coefficients
  * Causes an Level1 penalty -> lasso
  * If Normal distribution as a prior, it will cause a L2 penalty -> ridge regression

<p id="12"><b><font size="4">Asymmetric Laplace</b></font><a href="#0"><sup>Return</sup></a></p>
...
![Microarray log-ratio gene expression](http://web.stanford.edu/class/bios221/book/images/tcellhist.png)
...
The variance of a asymmetric laplace distribution is dependent on its mean (except for when mean=0, such as the case with a symmetric laplace distribution). The mean-variance relationship is common in physical measurements like microarray fluorescence intensity, mass spec peak heights or read counts from next-gen sequencing.
...
<p id="13"><b><font size="4">Infinite mixtures of poisson variables: the gamma-poisson distribution</b></font><a href="#0"><sup>Return</sup></a></p>
...
<p id="14"><b><font size="5">Choosing a data transformation based on a mixture model of your data: variance stabilizing transformations</b></font><a href="#0"><sup>Return</sup></a></p>
...
<p id="15"><b><font size="5">Exercises</b></font><a href="#0"><sup>Return</sup></a></p>
...
1. What does the M-step do, what does the E-step do?

The M-step takes the "unknown" parameters generated from the E step and uses them to modify/update the "known" parameters (probability of belong to group A or B).
...
2. What is the role of the algorithm arguments tolerance, miniter and maxiter?

Tolerance represents the minimum difference between log-likelihood estimates needed to loop. 
Miniter is the minimum number of times that we must interate between the E and M steps.
Maxiter is the maximum number of times we are allowed to interate between our E and M steps.

Class question: How do you think the authors determined tolerance? Is it always the same? Does it change? Is it related to a p-value? Thoughts?

3. Why do we need to compute loglik?
...
5. Compare the results of what we are doing to the output of the normalmixEM function from the mixtools package:
...
These paramters are close to the parameters of the function we just generated.  
...
#qqplot(qnbinom(p=ppoints(1e4), size=ofit$par[[1]], prob=ofit$par[[2]]), rgamma(1e4, shape=10, rate=3/2), main="",xlab = "gamma-Poisson size=9.687, prob=0.593", asp=1, cex=0.5, pch=16) + abline(a=0,b=1,col="blue")
...
Sam's artistic answer: They made it to look like a chromosome.

Nelson's more math based answer: Based on the generated graph it seems you can separate the points into a vertical parabola and linear line.

The chapter's answer: A simple artificial regression example with 2 latent classes, one independent variable (uniform on [0,10]), and three dependent variables with Gaussian, Poisson and Binomial distribution, respectively (generated using the command ?NPreg)
...
#Sam's solution
#good for looking at each observation independently
...
#Nelson's/Texbook solution
#good for looking at data as a whole

#There were 95 cells classified as group 1 and clustered as group 1
#5 cells were classified as group 1 but clustered as group 2
#5 cells were classified as group 2 but clustered as group 1
# 95 cells were classifed as group 2 and clustered as group 2
#overall 95% of cells were classified correctly
...
#The summary method lists for each component the prior probability, 
#the number of observationsassigned to the corresponding cluster, 
#the number of observations with a posterior probability larger
...
#the summary of m1 shows us how the data were classified prior to 
#fitting and how they are classified after, the summary also tells 
#us the log likelihood, bayesian information criteria (BIC) which 
#is the used to help select the statistical model (the model with the lowest BIC is preferred), and the #Akaike's Information #Criteria (AIC) which is an estimator of the statistical model #quality
```
...
#plot with colour (based on fitting classification) and shape (based on true group prior to fitting)
NPreg = mutate(NPreg, gr = factor(class))
ggplot(NPreg, aes(x = x, y = yn, group = gr)) +
   geom_point(aes(colour = as.factor(m1@cluster), shape = gr)) +
